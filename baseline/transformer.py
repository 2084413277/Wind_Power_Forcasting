...
# -----------------------------
# 基于 Transformer Encoder 的多步预测模型
# -----------------------------
class TransformerForecaster(nn.Module):
    """
    使用 nn.TransformerEncoder 来对过去 seq_len 个时刻进行编码，再映射到 pred_len 维。
    """
    def __init__(
        self,
        input_dim=6,
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True   # 使用 (batch_size, seq_len, d_model)
        )
    ...
    def forward(self, x):
        ...
        enc_output = self.transformer_encoder(x)  # (batch_size, seq_len, d_model)

        # 取最后时刻 enc_output[:, -1, :]
        last_feat = enc_output[:, -1, :]          # (batch_size, d_model)

        # 映射到 pred_len
        pred = self.fc_out(last_feat)            # (batch_size, pred_len)
        return pred
...
# 训练函数
# -----------------------------
def train_transformer(model, train_loader, val_loader, device, num_epochs=50, lr=1e-3):
    ...
    for epoch in range(num_epochs):
        ...
        for x, y, _ in train_loader:
            ...
            # 只关心第 0 列 => shape (batch_size, pred_len)
            y = y[..., 0].to(device)
...
def save_target_predictions_with_time(
    ...
    df_out.to_csv("24transformer12.csv", index=False)
    print("目标域预测结果及对应时间已保存至 24transformer12.csv")


# -----------------------------
# 主程序：仅使用目标域数据训练 Transformer
# -----------------------------
def main():
    ...
    # ========== 定义并训练 Transformer ==========
    print("Training Transformer model on target domain data...")
    ...
    train_transformer(model, train_loader, val_loader, device, num_epochs=num_epochs, lr=1e-3)
