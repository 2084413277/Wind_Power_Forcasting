...
# block-mask 函数
# -----------------------------
def random_block_mask(x, block_size=4, mask_prob=0.3):
    """
    x: (batch, seq_len, input_dim)
    在时间轴上随机选一段 block_size 的片段并置0，以 mask_prob 的概率执行遮蔽。
    """
    bsz, seq_len, in_dim = x.shape
    x_out = x.clone()
    ...
    for i in range(bsz):
        if np.random.rand() < mask_prob:
            start_idx = np.random.randint(0, seq_len - block_size + 1)
            x_out[i, start_idx:start_idx+block_size, :] = 0.0
    return x_out
...
# 源域训练 (带 block-mask)
# -----------------------------
def train_source_model_blockmask(
    ...
    for epoch in range(num_epochs):
        ...
        for x, y, _ in source_loader:
            ...
            # 这里对 x 做随机 block-mask
            x_masked = random_block_mask(x, block_size=block_size, mask_prob=mask_prob)

            x_in = source_input_layer(x_masked)
            feat = feature_extractor(x_in)
            pred = source_regressor(feat).view(y.size(0), pred_len, -1)
        ...
        print(f"[Source Training w/ blockmask] Epoch [{epoch+1}/{num_epochs}], Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
...
def main():
    ...
    source_data = source_df.to_numpy()  # (T,30)
    ...
    feature_extractor_src = FeatureExtractor(input_dim=32, proj_dim=128, kernel_size=3, dropout=0.2).to(device)
    source_regressor = Regressor(input_dim=32, output_dim=pred_len * 30).to(device)

    print("===== (A) Train on Source Domain with block-mask =====")
    train_source_model_blockmask(
        loader_source, loader_source_val,
        source_input_layer, feature_extractor_src, source_regressor,
    ...
    target_data_full = df_target.to_numpy()[:, :6]
    ...
    print("Loaded FeatureExtractor from source domain (trained w/ block-mask).")
    ...
    print("Plotting predictions vs actual on target test set (block-mask version)...")
